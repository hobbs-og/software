...
- `npm test` checks every contrast pair in every brand × theme it ships. A brand that fails is a design decision for Mark. A brand that ships only some themes lists them in `brandThemes` in `checks/contrast.json` (Mark, 2026-09-22: Rhinestone is light only); a brand left out is checked in every theme. Naming a brand Tier 1 has no mode for yet is a note, not a failure — it starts being checked when its export lands.
